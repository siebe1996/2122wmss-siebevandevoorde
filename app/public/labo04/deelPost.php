...
$number1 = isset($_GET['number1']) ? (string)$_GET['number1'] : random_int(0,100);
$number2 = isset($_GET['number2']) ? (string)$_GET['number2'] : random_int(0,100);
$som = '';
...
if ($moduleAction === 'processName'){
    if(trim($name) === ''){
        $message = 'Gelieve een aam in te vullen';
    }
    if ((filter_var($number1, FILTER_VALIDATE_INT) !== false) && (filter_var($number2, FILTER_VALIDATE_INT) !== false)){
        $som = $number1 + $number2;
    }
}
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
    ...
    <fieldset>
        ...
        <dl class="clearfix">
            ...
            <dt><label for="prijs">Prijs</label></dt>
            <dd class="text"><input type="text" id="prijs" name="prijs" value="<?php echo htmlentities($prijs); ?>" class="input-text" /></dd>
